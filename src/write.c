...
int put_data_obj(rcComm_t *conn, const char *path, rodsPath_t *rods_path,
                 char *default_resource, int flags, baton_error_t *error) {
    char *tmpname  = NULL;
    ...
    if (flags & VERIFY_CHECKSUM) {
        ...
        logmsg(DEBUG, "Server will validate checksum for '%s'",
               rods_path->outPath);
        status = chksumLocFile(tmpname, checksum, default_scheme);
        addKeyVal(&obj_open_in.condInput, VERIFY_CHKSUM_KW, checksum);
    }
